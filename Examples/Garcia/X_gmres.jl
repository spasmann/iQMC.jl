...
Uses xgmres_base which is completely oblivious to these things.
...
kl\\_store: You have the option (don't do it!) of giving me some room
         ...
         the way to do this is ```X_store=kstore(n,"gmres")``` where n
         is the number of unknows. I call this myself in the initialization
         phase if you don't do it ahead of me.

         Be very careful with this. X_store is use to store the solution
         to avoid overwriting the initial iterate. This means that
         two calls to X_gmres with the same X_store will step on the
...
julia> gout = X_gmres(x0, b, atv, V, 1.e-10; pdata = A);
...
julia> gout = X_gmres(u0, f, integop, V, 1.e-10; pdata = pdata);

julia> gout32 = X_gmres(u0, f, integop, V32, 1.e-10; pdata = pdata);
...
function X_gmres(
    ...
    (X_store !== nothing) || (X_store=kstore(n,"gmres"))
        y0 = X_store[1]
        y0 .= x0
        rhs = X_store[2]
        rhs .= b
   ...
   finalnorm=norm(b - atv(sol,pdata))
    println("final internal norm = $finalnorm")
...
Builds a matrix-vector product to hand to xgmres_base or bicgstab_base. 
...
"""
xgmres_base(x0, b, atv, V, eta, pdata; orth="cgs2", lmaxit=-1)
...
"""
function xgmres_base(x0, b, atv, V, eta, pdata; orth = "cgs2", lmaxit = -1)
